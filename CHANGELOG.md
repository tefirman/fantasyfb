...
### Removed
- **Dead V1 CLI flags `--games` and `--basaloppstringtime`** (#85): both were parsed and accepted but silently ignored downstream, `League.load_parameters()` explicitly documented them as "accepted for backward compatibility with the V1 CLI but ignored downstream." A user passing either flag would reasonably expect their projections to change; nothing happened. Removed from `cli.py`'s argument parsing (including the `basaloppstringtime` comma-split/validation block), the corresponding `League.__init__`/`load_parameters` parameters and placeholder `self.reference_games`/`self.basaloppstringtime` attributes, and their rows in the CLI reference docs. **Breaking change:** passing either flag now raises optparse's "no such option" error instead of silently doing nothing. Fixes #83.

### Fixed
- **`MoveAnalyzer.possible_pickups` crashed with a `TypeError` for any healthy drop candidate** (#86): `"'>=' not supported between instances of 'NoneType' and 'int'"`. `until` is `NaN`/`None` by default for a player with no active injury designation (set only when `PlayerDataManager.add_injuries` finds one), but the "is this player out through the current week" check compared it directly against an int with no null guard, unlike the free-agent-availability filter two lines earlier in the same method, which already handled this correctly via `until.isnull() | (until < 17)`. A null `until` now takes the same branch a healthy player should: only free agents within 0.5 WAR of the player being dropped are considered, rather than "any free agent is an upgrade" (reserved for a player who's actually out and contributing nothing this week). First test coverage added for `MoveAnalyzer`, which is how this went unnoticed.
- **2025+ depth-chart schema labeled kickers "PK" instead of "K"** (follow-up to #78): nflreadpy's newer depth-chart feed uses `pos_abb` value `"PK"` for kickers, while everywhere else in fantasyfb (the `players` DataFrame, the legacy pre-2025 depth-chart schema, `MatchupModel`'s `_STARTER_SLOTS`/`_DEFAULT_WEIGHTS`) keys on `"K"`. `get_depth_charts()` didn't normalize this, so the live feed returned zero `"K"` rows and `PlayerDataManager.add_depth_charts`'s name+position fallback could never match a kicker, silently defaulting every one of them to `string=2.0`. Currently harmless in practice since `K`'s `gamma` weight is `0.0` (the string penalty never fires for kickers regardless), but would misfire the moment that weight is tuned away from 0. `get_depth_charts()` now normalizes `"PK"` to `"K"` right after the schema-specific rename.
- **Already-started players in a completed game showed as benched during a live week** (follow-up to #18): `LineupOptimizer._handle_live_week_lineup` correctly identifies players whose game has finished and who were active (`selected_position != "BN"`) in the real platform lineup, but only ever used that set to compute *remaining* roster capacity -- nothing set `players["starter"] = True` for them, so they fell back to the `starter=False` default despite being legitimately active. That silently broke every downstream consumer of the starter flag: `League._apply_live_week_actuals`'s real-score blending never substituted in their actual points, and `season_sims` never counted their points toward the week's team total at all. First test coverage added for `LineupOptimizer` (previously untested), which is how this went unnoticed.
- **Depth-chart rows fanned out into duplicates for any player with a secondary role** (follow-up to #78): nflreadpy's depth chart has one row per (player, role) -- a WR who also returns punts, or an RB who also returns kicks, has multiple rows sharing the same `player_id_sr`. `get_depth_charts()` joined these onto the players DataFrame with no dedup, so 200+ players league-wide were duplicated downstream. That corrupted `LineupOptimizer`'s per-position slot-filling: two rows for one real player could each independently pass the "not yet a starter" filter, silently consuming a roster slot meant for a different, lower-scoring teammate. A new `_dedupe_depth_chart()` collapses to one row per `player_id_sr`, preferring the fantasy-relevant offensive/kicking role over a special-teams-only one (a naive "lowest string wins" rule doesn't work -- a KR/PR string is often numerically lower than a player's real offensive string).
- **`MatchupModel`'s depth-chart string penalty crushed real WR2/WR3/RB2/TE2 starters** (#78): the flat `max(0, string - 1)` penalty treated nflreadpy's `pos_rank` (a full ordinal depth ranking, 1-9+) as a starter/backup flag for every position, when that assumption only holds for QB -- real NFL offenses routinely start 2-3 WRs and use multiple RBs. Legitimate starting WR2s/WR3s/RB2s/TE2s were penalized as if they were backups, corrupting `points_avg`, lineup selection, and `season_sims` for every week of the season. Replaced with a per-position "real starter slots" cutoff (QB: 1, RB: 2, WR: 3, TE: 2) below which no penalty applies, tapering in beyond that. Also fixed `sim/backtest.py`'s hardcoded `string=1.0`, which meant the walk-forward fit never actually exercised the string penalty: `get_depth_charts()` now accepts an optional `(season, week)` to look up real historical depth-chart data (available back to at least 2015, contrary to a since-removed comment claiming otherwise).
- **Current-week matchup projections ignored points already on the board mid-slate** (#18): during a live week, the season simulator summed every starter's pre-game `points_avg` for the current week even after their NFL game had finished, so a matchup's projected score and win probability stayed frozen at their pre-kickoff values as Sunday unfolded. Added `League._apply_live_week_actuals`: for the active season's current week, any starter whose NFL game has finished gets their actual fantasy points substituted in as `points_avg` with `points_stdev` set to 0, so that part of the matchup becomes deterministic and only pending games still carry variance.
