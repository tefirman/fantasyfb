...
def get_depth_chart(team_abbrev: str):
    ...
    corrections = pd.read_csv(
        "https://raw.githubusercontent.com/tefirman/FantasySports/main/res/football/name_corrections.csv"
    )
    depth = pd.merge(
        left=depth,
...
def get_all_depth_charts():
    ...
    """
    teams = pd.read_csv(
        "https://raw.githubusercontent.com/tefirman/FantasySports/main/res/football/team_abbrevs.csv"
    )
...
class Schedule:
    ...
    def add_weeks(self):
        ...
        self.schedule.loc[mask, "week"] = self.schedule.loc[mask, "week_num"].astype(
            int
        )
    ...
    def add_elo_columns(self, qbelo: bool = False):
        ...
        if qbelo:
            qb_elos = get_qb_elos(
                self.schedule.season.min(), self.schedule.season.max()
            )
            for team_num in ["1", "2"]:
                self.schedule = pd.merge(
                    left=self.schedule,
...
def get_bulk_stats(
    ...
    to_save = (
        path is not None
        and (~s.schedule.boxscore_abbrev.isin(stats.game_id.unique())).any()
    )
    ...
    if to_save:
        stats.to_csv(path, index=False)
    stats = stats.loc[
        stats.game_id.isin(s.schedule.boxscore_abbrev.tolist())
    ].reset_index(drop=True)
    return stats
...
def get_qb_elos(
    ...
    stats = get_bulk_stats(
        start - 3, 1, finish, 50, True, "GameByGameFantasyFootballStats.csv"
    )
