...
class PlayerConfig:
    ...
    earliest: Dict[str, int] = field(default_factory=lambda: {
        "QB": 202201, "RB": 202201, "WR": 202201,
        "TE": 202201, "K": 202201, "DEF": 202201
    })
    reference_games: Dict[str, int] = field(default_factory=lambda: {
        "QB": 15, "RB": 12, "WR": 12,
        "TE": 12, "K": 10, "DEF": 12
    })
    weighting_factors: pd.DataFrame = field(default_factory=lambda: pd.DataFrame({
...
class LeagueConfig:
    ...
    def for_bestball(cls, platform: str = "draftkings") -> 'LeagueConfig':
        ...
        if platform.lower() in ["dk", "draftkings"]:
            ...
            config.scoring_config = ScoringConfig(
                pass_yds=0.04, pass_td=4.0, pass_300_bonus=3.0, int_thrown=-1.0,
                rush_yds=0.1, rush_td=6.0, rush_100_bonus=3.0,
                rec_yds=0.1, rec=1.0, rec_td=6.0, rec_100_bonus=3.0,
                fum_lost=-1.0
            )
            ...
            config.num_playoff_teams = 2
            config.scoring_config = ScoringConfig(
                pass_yds=0.04, pass_td=4.0, int_thrown=-1.0,
                rush_yds=0.1, rush_td=6.0,
                rec_yds=0.1, rec=0.5, rec_td=6.0,
                fum_lost=-2.0
            )
            config.roster_config = RosterConfig(
    ...
    def for_sfb(cls) -> 'LeagueConfig':
        ...
        config.scoring_config = ScoringConfig(
            ...
            rush_yds=0.1, rush_att=0.25, rush_td=6.0, rush_1d=0.5,
            rec_yds=0.1, rec=0.75, rec_td=6.0, rec_1d=0.5,
            te_rec_bonus=0.75, te_1d_bonus=1.0,
            ret_yds=0.2, ret_td=10.0,
            two_pt=2.0,
