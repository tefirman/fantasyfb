...
class SeasonSimulator:
    ...
    def season_sims(
        ...
        schedule_with_projections = self._merge_schedule_projections(
        ...
        standings_results = self._aggregate_standings_results(
            standings_sims, postseason
        )
    ...
    def _generate_weekly_projections(
        self, players: pd.DataFrame, schedule: pd.DataFrame
    ) -> pd.DataFrame:
            ...
            if not team_projections.empty:
                if projections.empty:
                    ...
                    projections = pd.concat(
                        [projections, team_projections], ignore_index=True
                    )
    ...
    def _set_starters(self, players: pd.DataFrame, week: int) -> pd.DataFrame:
        ...
        for flex_pos in flex_positions:
            if flex_pos in roster_spots.position.values:
                num_flex = int(
                    roster_spots.loc[roster_spots.position == flex_pos, "count"].sum()
                )
    ...
    def _merge_schedule_projections(
        self, schedule: pd.DataFrame, projections: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        schedule["points_avg_1"] = schedule["points_avg_1"].fillna(0.0) + schedule[
            "score_1"
        ].fillna(0.0)
        schedule["points_avg_2"] = schedule["points_avg_2"].fillna(0.0) + schedule[
            "score_2"
        ].fillna(0.0)
    ...
    def _simulate_games(
        ...
        schedule_sims["sim_1"] = np.random.normal(
            schedule_sims["points_avg_1"], np.sqrt(schedule_sims["points_var_1"])
        )
        schedule_sims["sim_2"] = np.random.normal(
            schedule_sims["points_avg_2"], np.sqrt(schedule_sims["points_var_2"])
        )
    ...
    def _calculate_standings(
        ...
        team1_results = schedule_sims[
            ["sim_num", "week", "team_1", "sim_1", "win_1"]
        ].rename(columns={"team_1": "team", "sim_1": "points", "win_1": "wins"})
        team2_results = schedule_sims[
            ["sim_num", "week", "team_2", "sim_2", "win_2"]
        ].rename(columns={"team_2": "team", "sim_2": "points", "win_2": "wins"})
    ...
    def _simulate_bestball_season(
        ...
        season_sims["points_sim"] = np.random.normal(
            season_sims["points_avg"], season_sims["points_stdev"]
        )
        ...
        standings = team_scores.sort_values(
            ["sim_num", "points_sim"], ascending=[True, False]
        ).reset_index(drop=True)

        standings["place"] = standings.groupby("sim_num").cumcount() + 1
        standings["playoffs"] = (
            standings["place"] <= self.league.settings["num_playoff_teams"]
        ).astype(float)
        ...
        return final_standings.sort_values("playoffs", ascending=False).reset_index(
            drop=True
        )
    ...
    def _aggregate_schedule_results(self, schedule_sims: pd.DataFrame) -> pd.DataFrame:
        ...
        return schedule_results.sort_values(["week", "team_1", "team_2"]).reset_index(
            drop=True
        )

    def _aggregate_standings_results(
        self, standings_sims: pd.DataFrame, postseason: bool
    ) -> pd.DataFrame:
        ...
        per_game_stats = (
            all_scores.groupby("team").agg({"points": ["mean", "std"]}).reset_index()
        )
        per_game_stats.columns = ["team", "per_game_avg", "per_game_stdev"]
        per_game_stats["per_game_fano"] = (
            per_game_stats["per_game_stdev"] / per_game_stats["per_game_avg"]
        )
        ...
        sort_col = (
            "winner" if postseason and "winner" in standings_agg.columns else "playoffs"
        )
